...
class SampleDataset:
    ...
    Y_t = \gamma \exp(-(x + offset)^2 / l) + \eta_{1t}, where \eta_{1t} \sim Gau(0, R_{1t})
    S_t = Y_t + X_t + \eta_{2t}, where \eta_{2t} \sim Gau(0,R_{2t})
    Z_t = Y_t + \epsilon_t, where \epsilon_t \sim Gau(0, R_{3t})


    Attributes:
          Z: generated dataset
          frames_input: length of the observed sequences
          frames_output: length of the sequences to predict
    """
    ...
    def prepare_data(self, n, T, gamma, l, offset, total):
        ...
        :param offset: parameter in the linear dstm model
        :param total: total number of training samples
        :return: the tensor of size (total x T x 1 x n x n)
        """
    ...
    def __getitem__(self, idx):
        ...
        input = Z[self.warm_start_ind:(self.warm_start_ind +
                                       self.frames_input),
                  ...]  # observed sequences
        output = Z[(self.warm_start_ind +
                    self.frames_input):(self.warm_start_ind +
                                        self.frames_input +
                                        self.frames_output),
...
class SampleDatasetDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        if stage == 'fit' or stage is None:
            self.data = SampleDataset(self.n, self.T, self.gamma, self.l,
                                      self.offset, self.total,
                                      self.frames_input, self.frames_output,
                                      self.warm_start_ind)
            self.training_data, self.validation_data = random_split(
                self.data,
                [self.training_data_size, self.validation_data_size])
...
class CLSTM_cell(pl.LightningModule):
    ...
    def __init__(self, shape, input_channels, filter_size, num_features,
        ...
        if 4 * self.num_features < 32:
            print(
                "GroupNorm will not be applied, require more output channels to apply GroupNorm!"
            )
        else:
            print("GroupNorm will be applied!")
            self.groupnorm = nn.GroupNorm(4 * self.num_features // 32,
                                          4 * self.num_features)
    ...
    def forward(self, seq_len, inputs, hidden_state):
        ...
        if self.dropout_rate == 0:
            ...
            self.dropout_layer = BayesianDropout(
                self.dropout_rate,
...
class ConvCell(pl.LightningModule):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding,
                 dropout_rate):
        super().__init__()
        self.pooling_layer = nn.MaxPool2d(kernel_size=kernel_size,
                                          stride=stride,
                                          padding=padding)
...
class Encoder(pl.LightningModule):
    ...
    def forward(self, inputs, frames_input):
        ...
        for i in range(1, self.blocks + 1):
            ...
            inputs, state_stage = cur_rnn(seq_len=frames_input,
                                          inputs=inputs,
                                          hidden_state=None)
...
class Decoder(pl.LightningModule):
    ...
    def forward(self, hidden_states, frames_output):
        ...
        res = []
        inputs, _ = cur_rnn(seq_len=frames_output,
                            inputs=None,
...
class LightningConvLstm(pl.LightningModule):
    ...
    def simple_plot(self, x, pred, text='Train/Pred'):
        ...
        self.logger.experiment.add_figure(
            "True" + text, plot_spatio_temporal_data(x.squeeze(1)),
            self.current_epoch)
        self.logger.experiment.add_figure(
            "Pred" + text, plot_spatio_temporal_data(pred.squeeze(1)),
            self.current_epoch)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        naive_predictor = torch.ones_like(targetVar).permute(
            3, 4, 0, 1, 2) * targetVar.mean(dim=(-1, -2))
    ...
    def validation_epoch_end(self, outputs):
        # calculate the average loss
        avg_loss = torch.stack([x['loss'] for x in outputs]).mean()
        avg_naive_predictor_loss = torch.stack(
            [x['naive_predictor_loss'] for x in outputs]).mean()

        # logging using tensorboard logger
        self.logger.experiment.add_scalar('Loss/Validation', avg_loss,
                                          self.current_epoch)
        self.logger.experiment.add_scalar('Loss/Naive_predictor',
                                          avg_naive_predictor_loss,
                                          self.current_epoch)
...
if __name__ == "__main__":
    ...
    output_cnn = ConvCell(in_channels=128,
                          out_channels=1,
                          kernel_size=1,
                          stride=1,
                          padding=0,
    ...
    model = LightningConvLstm(encoder_rnns, decoder_rnns, output_cnn,
                              args.frames_input, args.frames_output,
                              args.mc_dropout, args.learning_rate)
    # attrs = vars(model)
    # print(', '.join("%s: %s" % item for item in attrs.items()))
    ...
    dm = SampleDatasetDataModule(
        ...
        frames_output=args.frames_output,
        warm_start_ind=args.warm_start_ind)
