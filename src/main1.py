...
class SampleDataset:
    ...
    def prepare_data(self, n, T, gamma, l, offset, total):
        ...
        weights_matrix = gamma * np.exp(-(distance_matrix_list + offset)**2 / l) # create a matrix, each row stores the weights matrix between it and every spatial location
        ...
        for i in range(total):
            ...
            for t in range(1, T):
               ...
               Y[:, t, i] = np.dot(weights_matrix, (Y[:, (t - 1), i])[:, None]).ravel()
               Z[:, t, i] = Y[:, t, i]
    ...
    def __getitem__(self, idx):
        ...
        input = Z[self.warm_start_ind:(self.warm_start_ind+self.frames_input), ...] # observed sequences
        output = Z[(self.warm_start_ind+self.frames_input): (self.warm_start_ind+self.frames_input+self.frames_output), ...] # sequences to predict
        frozen = input[-1]
...
class SampleDatasetDataModule(pl.LightningDataModule):
    def __init__(self, n, T, gamma, l, offset, batch_size, training_data_size, validation_data_size, frames_input, frames_output, warm_start_ind=0):
        super().__init__()
        self.n = n
    ...
    def setup(self, stage=None):
        ...
        if stage == 'fit' or stage is None:
            self.data = SampleDataset(self.n, self.T, self.gamma, self.l, self.offset, self.total, self.frames_input, self.frames_output, self.warm_start_ind)
...
class Encoder(pl.LightningModule):
    ...
    def forward(self, inputs, frames_input):
        ...
        for i in range(1, self.blocks + 1):
            ...
            inputs, state_stage = cur_rnn(seq_len=frames_input, inputs = inputs, hidden_state=None)
...
class Decoder(pl.LightningModule):
    ...
    def forward(self, hidden_states, frames_output):
        cur_rnn = getattr(self, 'rnn1')
        res = []
        inputs, _ = cur_rnn(seq_len=frames_output, inputs=None, hidden_state=hidden_states[0])
        res.append(inputs)
        for i in list(range(1, self.blocks)):
            cur_rnn = getattr(self, 'rnn' + str(i+1))
            inputs, _ = cur_rnn(seq_len=frames_output, inputs=inputs, hidden_state=hidden_states[i])
            res.append(inputs)
...
class ED(pl.LightningModule):
    ...
    def forward(self, input, frames_input, frames_output):
        state = self.encoder(frames_input=frames_input, inputs=input)
        output = self.decoder(frames_output=frames_output, hidden_states=state)
        return output
...
class LightningConvLstm(pl.LightningModule):
    ...
    def __init__(self, encoder_rnns, decoder_rnns, output_cnn, frames_input, frames_output, mc_dropout=5, learning_rate=1e-3):
    ...
    def forward(self, x):
        ...
        return self.net(x, self.frames_input, self.frames_output)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser()
    parser.add_argument('--root', type=str, default='../../data/')
    parser.add_argument('--dropout_rate', type=float, default=0.2, help='dropout rate for all layers')
    ...
    ## case 3
    encoder_rnns = [CLSTM_cell(shape=(10, 10), input_channels=1, filter_size=3, num_features=64, dropout_rate=args.dropout_rate),
                    CLSTM_cell(shape=(10, 10), input_channels=64, filter_size=3, num_features=64, dropout_rate=args.dropout_rate)]
    decoder_rnns = [CLSTM_cell(shape=(10, 10), input_channels=1, filter_size=3, num_features=64, dropout_rate=args.dropout_rate),
                    CLSTM_cell(shape=(10, 10), input_channels=64, filter_size=3, num_features=64, dropout_rate=args.dropout_rate)
                    ]
    ...
    model = LightningConvLstm(encoder_rnns, decoder_rnns, output_cnn, args.frames_input, args.frames_output, args.mc_dropout, args.learning_rate)
    # attrs = vars(model)
    # print(', '.join("%s: %s" % item for item in attrs.items()))
    ...
    dm = SampleDatasetDataModule(n=args.n, T=args.seq_len, gamma=0.2, l=1, offset=0, batch_size=args.batch_size, training_data_size=args.training_data_size, validation_data_size=args.validation_data_size, frames_input=args.frames_input, frames_output=args.frames_output, warm_start_ind=args.warm_start_ind)
